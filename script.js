jQuery(function($) {
    let email_flag = 0;// флаг для проверки что почта введена правильно
    let password_flag = 0;// ^флаг для пароля
    let name_flag = 0;
    ...
    $(".btn-auth").click(function(event){
        ...
        if ( email_flag == 1 && password_flag == 1 ){// проверка что поля почта и пароль заполнены правильно
     	...
     	$('.captcha-placeholder').html("").load("handlers/captcha.php");
    });
    $('.captcha-placeholder').load("handlers/captcha.php");
    ...
    $(".btn-reg").click(function(event){
        event.preventDefault();
        if ( email_flag == 1 && password_second_flag == 1 && name_flag == 1 ){// проверка что поля почта и пароль заполнены правильно
            $.ajax({
    ...
    $('.password_second').change(function() {
